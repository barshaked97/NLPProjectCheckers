...
class Trainer:
    ...
    def load_checkpoint(self):
        ...
        raw_model.load_state_dict(torch.load(os.path.join(self.config.ckpt_path, "checkpoint.ckpt")))
    ...
    def train(self, prt=True):
        ...
        def run_epoch(split):
            ...
            losses = []
            # totals_epoch = np.zeros(32, dtype=float)  # np.array of shape [60], for positions of age 0 to 59
            # hits_epoch = np.zeros(32, dtype=float)  # np.array of shape [60], for positions of age 0 to 59
            totals_epoch = 0
            hits_epoch = 0
            totals_epoch1 = 0
            ...
            for it, (x, y) in pbar:
                ...
                with torch.set_grad_enabled(is_train):
                    ...
                    # totals_epoch += np.array([torch.sum(age == i).item() for i in range(32)]).astype(float)
                    totals_epoch += y.size(0) * y.size(1)
                    # print(y.size(0))
                    # print(y.shape)
                    # totals_epoch1 += y.size(0)
                    y_hat = torch.argmax(logits, dim=-1, keepdim=False)  # [B, #task]
                    # print(y_hat)
                    hits = y_hat == y  # [B, #task]
                    # print(hits)
                    # print(torch.sum(hits), "\n\n")
                    # hits_epoch += np.array([torch.sum(hits * (age == i)).item() for i in range(32)]).astype(float)
                    hits_epoch += torch.sum(hits).item()
                ...
                if is_train:
                    ...
                    mean_acc = hits_epoch / totals_epoch
            ...
            if not is_train:
                ...
                test_acc = hits_epoch / totals_epoch
                if prt: 

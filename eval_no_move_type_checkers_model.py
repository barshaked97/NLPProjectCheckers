...
test_dataset = CharDatasetNoMoveType(kings=True, test=True)
mconf = GPTConfig(test_dataset.vocab_size, test_dataset.block_size, n_layer=8, n_head=8, n_embd=512)
model = GPT(mconf)

load_res = model.load_state_dict(torch.load("C:/Users/Arik/Documents/uni/NLP/Project/NLPProjectCheckers/ckpts/gpt_no_move_type.ckpt"))
...
def check_legal_move(game_env, move):
    ...
    if l in moves_list:
        ...
        a, b, done = game_env.step(legal_next_states[id], (l[1][0],l[1][1]))
...
for whole_game in bar:
    ...
    # for length_of_partial_game in range(1, length_of_whole_game):
        ...
        if not is_Legal(prev_prev_word, prev_word, new_word):
            ...
            while not swap_player:
                partial_move = move_global[:3] if move_global[0]=="K" else move_global[:2]
                _, swap_player = check_legal_move(game_env_global, partial_move)
                move_global = move_global[2:] if move_global[0]=="K" else move_global[1:]
                length_of_partial_game += 1
            # _, _ = check_legal_move(game_env_global, move_global)
            if len(move_global) >= 2 and move_global[1] == "K":
                length_of_partial_game += 1
            ...
            if (move[0] == "K" and len(move) == 3) or (len(move) == 2 and move[0] != "K"):
                # print(completion)
                # print(move)
                ...
                if legal and not swap_player:
                    ...
                    #     moves_per_game.append(len(game_env_local.history))
                    #     break
                    move_global = whole_game[start_index - 1: start_index + 10]
                    # length_of_partial_game = start_index + id + 1
                    swap_player = False
                    # length_of_partial_game += 1
                    count = 0

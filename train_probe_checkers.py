...
if __name__ == '__main__':
    ...
    folder_name = f"battery_checkers/{args.exp}"
    ...
    if args.random:
        ...
        load_res = model.load_state_dict(torch.load("./ckpts/gpt_no_move_type.ckpt"))
    ...
    for x, y in tqdm(loader, total=len(loader)):
        ...
        # a = OthelloBoardState()
        a = Checkers.Checkers(neural_net=None)
        # print(tbf[:valid_until])
        ...
        # if len(property_container) != len(act_container):
        #     print(tbf)
        #     print(properties)
        #     exit()
    ...
    if args.twolayer:
        probe = BatteryProbeClassificationTwoLayer(device, probe_class=probe_class, num_task=32, mid_dim=args.mid_dim)
    else:
        probe = BatteryProbeClassification(device, probe_class=probe_class, num_task=32)
    ...
    sampler = None
    train_loader = DataLoader(train_dataset, shuffle=False, sampler=sampler, pin_memory=True, batch_size=20, num_workers=0)
    test_loader = DataLoader(test_dataset, shuffle=True, pin_memory=True, batch_size=20, num_workers=0)

    max_epochs = args.epo
    t_start = time.strftime("_%Y%m%d_%H%M%S")
    tconf = TrainerConfig(
        max_epochs=max_epochs, batch_size=20, learning_rate=3e-3,
        betas=(.9, .999),
        lr_decay=True, warmup_tokens=len(train_dataset)*5,
        final_tokens=len(train_dataset)*max_epochs,
        num_workers=0, weight_decay=0.,
        progress=0,
